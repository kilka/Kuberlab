...
#
# Usage:
#   cleanup-orphans.sh         # Full cleanup (for destroy)
#   cleanup-orphans.sh keyvault-only  # Only clean soft-deleted Key Vaults (for deploy)

set -euo pipefail

# Check for keyvault-only mode
KEYVAULT_ONLY=${1:-}
...
if [ "$KEYVAULT_ONLY" = "keyvault-only" ]; then
    echo -e "${BLUE}Checking for soft-deleted Key Vaults only...${NC}"
else
...
# Only run full cleanup if not in keyvault-only mode
if [ "$KEYVAULT_ONLY" != "keyvault-only" ]; then
